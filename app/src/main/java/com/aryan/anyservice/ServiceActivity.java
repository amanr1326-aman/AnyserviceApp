    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        checkoutButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                intent.putExtra("totalPrice",price+visitCost);
            ...
            if(result.get("result").equals("Success")) {
                if (method.equals("get_user_services")) {
                    ...
                    runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                            company.setText(result.get("company").toString());
                            rating.setRating(Float.parseFloat(String.valueOf(result.get("rating"))));
                            address.setText(result.get("address").toString());
                            categories.setText(result.get("categories").toString());
                            try {
                                byte[] decodedString = Base64.decode(result.get("image").toString(), Base64.DEFAULT);
